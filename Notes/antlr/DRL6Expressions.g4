...
conditionalExpression
    :   conditionalOrExpression ternaryExpression?
    ;
...
fullAnnotation
    : AT ID ( DOT ID )* annotationArgs
    ;

annotationArgs 
    : LEFT_PAREN ((ID EQUALS_ASSIGN) | annotationValue)? RIGHT_PAREN
    ;
...
annotationElementValuePair 
    : ID EQUALS_ASSIGN annotationValue
    ;
...
conditionalOrExpression
    : conditionalAndExpression  ( DOUBLE_PIPE fullAnnotation? conditionalAndExpression )*
    ;

conditionalAndExpression
    : inclusiveOrExpression ( DOUBLE_AMPER fullAnnotation? inclusiveOrExpression )*
    ;
...
inExpression
    : relationalExpression (
        (not_key in_key) expression (COMMA expression )* RIGHT_PAREN 
        | in_key LEFT_PAREN expression (COMMA expression )* RIGHT_PAREN 
    )?
    ;

relationalExpression
    : shiftExpression ( ( operator | LEFT_PAREN ) )*
    ;

orRestriction
    : andRestriction ( (DOUBLE_PIPE fullAnnotation? andRestriction) )* EOF?
    ;

andRestriction
    : singleRestriction ( (DOUBLE_AMPER fullAnnotation? operator) )*
    ;

singleRestriction
    :  operator ( (squareArguments shiftExpression) )
    |  LEFT_PAREN orRestriction RIGHT_PAREN 
    ;

shiftExpression
    : additiveExpression ( (shiftOp) )*
    ;
...
unaryExpressionNotPlusMinus
    ...
    | (( ID COLON ) | (ID UNIFY ))? (xpathSeparator ID) (selector)* (INCR|DECR)?
    ; 

castExpression
    :  (LEFT_PAREN primitiveType)
    |  (LEFT_PAREN type) 
    ;
...
primary
    ...
    | (ID) ((DOT ID) | ((DOT|NULL_SAFE_DOT) LEFT_PAREN) | (HASH ID) | (NULL_SAFE_DOT ID))* (identifierSuffix)?
    ;
...
identifierSuffix
    ...
    | ((LEFT_SQUARE))+ // can also be matched by selector, but do here
    | arguments
    ;
...
createdName
    : ID typeArguments? (DOT ID typeArguments?)* | primitiveType
    ;
...
arrayCreatorRest
    : LEFT_SQUARE ( RIGHT_SQUARE (LEFT_SQUARE RIGHT_SQUARE)* arrayInitializer | expression RIGHT_SQUARE ((LEFT_SQUARE RIGHT_SQUARE))* )
    ;

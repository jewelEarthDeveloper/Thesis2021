...
In our pilot study, we created a simple approximation of the Drools language to create our first projections. 
...
Following and empty line, there is a vertical listing of the \texttt{FactDeclaration} nodes stored in the ``facts'' child of the \texttt{File} node.
...
Each \texttt{FactDeclaration} node will be shown on a single line starting with the word ``fact'' followed by its name, then, between parentheses, a horizontal, comma separated list of its child \texttt{FactProperty} nodes.
...
Next, we show one of the \texttt{AbstractCondition} Concepts, in the case the \texttt{AndCondition} Concept.
This displays the two \texttt{FactSelector} child nodes separated by ``and'' and surrounded by parentheses.

The editor for the \texttt{FactSelector} Concept displays the fact child, which is a \texttt{FactDeclarationSmartRef} node, for which we do not show it's editor here, but in this case shows the name of the \texttt{FactSelector} node that it is referencing.
...
The \texttt{FactDeclaration} nodes called ``Dossier'', ``Episode'', and ``Milestone'' each have 2 child \texttt{FactProperty} nodes, whilst ``DroolsContext'' has none.
...
The \texttt{Rule} node ``[WVGGZ/CM] start CM Procedure'' points to the ``Dossier'' \texttt{FactDeclaration} node, as well as having 2 predicates.
...
The first is an \texttt{OrCondition} node which contains an \texttt{ExistsCondition} node in its left-hand side and an \texttt{AndCondition} node on the right.
...
What is does is when it encounters the possibility to input an \texttt{ExistsCondition} node in a menu, it replaces it with the default menu for the \linebreak\texttt{FactDeclarationSmartRef} Concept.
MPS handles SmartRefs in menus by showing all the possible reference items available.
When the user picks the reference item this code will then create a new \texttt{ExistsCondition} node with a new \texttt{FactSelector} node containing the chosen \texttt{FactDeclarationSmartRef} node, to insert at that point in the AST.
This saves several keystrokes, as otherwise we manually first had to insert a \texttt{ExistsCondition} node followed by a \texttt{FactSelector} node before filling the \texttt{FactDeclaration} child.
...
This scope Constraint made it much easier to select \texttt{FactProperty} nodes in a \linebreak\texttt{Predicate} node, as indicated in figure \ref{fig:RSDConstraint}.
...
The figure shows that before adding the scoping constraint, it showed a list with dozens of potential \texttt{FactProperty} nodes, that represented all the \linebreak\texttt{FactProperty} nodes in the model.
...
This first finds the relevant \texttt{FactDeclaration} node from the \texttt{FactSelector} node we are at in the AST.
...
\paragraph{\texttt{RuleFile}} The \texttt{RuleFile} level statements contain \texttt{FactDeclaration} nodes, \texttt{Global} nodes and \texttt{Rule} nodes.
It also contains semantically unimportant empty lines.

\paragraph{FactDeclaration} A \texttt{FactDeclaration} has a \texttt{type} property.
...
\paragraph{FactProperty} In RSD, we had \texttt{FactProperty} nodes as children of \texttt{Facts}.
...
We added a smart reference so that \texttt{Rule}s can easily use them.
...
\paragraph{\texttt{Rule}} Our \texttt{Rule}s have three children: an \texttt{AttributeCollection}, a Right-hand side and a list of Conditions that make up the Left-hand side.
...
\paragraph{\texttt{RuleVariables}} The \texttt{FactDeclaration} reference of a \texttt{FactSelector} and the \texttt{FactProperty} reference of a \texttt{FieldConstraint} are bound to \texttt{RuleVariable} nodes.
...
To implement this, we made the right-hand side of the \texttt{Rule} a single \texttt{StatementList}.
...
\paragraph{\texttt{AttributeCollection}} the \texttt{AttributeCollection} is a container to hold all the attributes that apply to a \texttt{Rule}.
...
On line 2 in figure \ref{fig:Rule}, we can see an example of the \texttt{SalienceAttribute} added to a \texttt{Rule} on line 2.
...
\paragraph{\texttt{FactSelector}} This always has a reference to a \texttt{FactDeclaration}.
These \texttt{Fact} nodes are named ``Program'' in line 4 of figure \ref{fig:Rule} and ``Student'' and ``Result'' from line 5.

Optionally, the \texttt{FactSelector} can be bound to a variable.
In figure \ref{fig:Rule}, line 5, the \texttt{FactSelector} referencing the \texttt{Student} \texttt{FactDeclaration} reference is bound to the \texttt{\$s} \texttt{RuleVariable}.

The \texttt{FactSelector} also contains a list of constraints on \texttt{FactProperty} references, all of which must return true for the \texttt{FactSelector} to return \texttt{true}.
...
The \texttt{FieldConstraints} places restrictions on \texttt{FactProperty} references.
...
\begin{itemize}    
    ...
    \item \texttt{VariableRestrictions}: These can be \texttt{Global} references, \texttt{RuleVariable} references referring to \texttt{FactDeclaration} references from the \texttt{FactSelector}, or \texttt{RuleVariables} from other \texttt{FieldConstraints}.

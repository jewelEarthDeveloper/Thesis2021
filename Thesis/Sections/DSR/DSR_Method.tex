...
What is does is when it encounters the possibility to input an \texttt{ExistsCondition} node in a menu, it replaces it with the default menu for the \texttt{FactDeclarationSmartRef} Concept.
...
The figure shows that before adding the scoping constraint, it showed a list with dozens of potential \texttt{FactProperty} nodes, that represented all the \texttt{FactProperty} nodes in the model.

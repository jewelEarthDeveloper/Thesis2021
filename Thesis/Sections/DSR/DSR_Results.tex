...
This approach, however, requires running a macro on the code file to migrate \texttt{Facts}, \texttt{FactProperties} and \texttt{Files} to \texttt{FilteredFacts}, \linebreak\texttt{FilteredFactProperties}, and \texttt{FilteredFiles}.
...
The reason behind this projection is that the previous table does not give any visual cues as to how \texttt{Rules} are related.
...
From the ``on delete'' section of the vertical section, we loop through all the rules to remove the references and rearrange the conditions in the AST.
...
The major drawback of this design is that editing a rule with a yet non-existent selection criteria became very clunky.

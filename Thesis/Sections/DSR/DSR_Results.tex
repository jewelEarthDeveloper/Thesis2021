...
\texttt{FactDeclaration} references contained by \texttt{NotConditions} and \texttt{FactProperty} references that are part of a \texttt{NotPredicate} appear highlighted in Red.
...
The \texttt{Rule} count and \texttt{FactDeclaration} count were added to the \texttt{File} editor using Read-Only Model Access to count the descendants of the \texttt{File} that are \texttt{Rule} nodes and \texttt{FactDeclaration} nodes.
The \texttt{Rule} editor was adjusted only to show the \texttt{Rule} nodes title and, again using the model access, the count of the descendants of the \texttt{Rule} that were \texttt{FactDeclaration} references.

Whilst this may look like a report that any language workbench could create, the \texttt{File} name and the names of the \texttt{Rule} nodes are editable in this projection.
...
We created a projection where if the developer filtered by a \texttt{FactDeclaration} or a \texttt{FactProperty}, the projection would filter out all \texttt{Rule} nodes that did not contain the item.
...
The first is an intention associated with a \texttt{FactDeclaration}.
...
As the \texttt{FactProperty} is a child of a \texttt{FactDeclaration}, we see both intentions.
...
In our first approach at the filtering, we failed on this count by invasively adding properties to \texttt{FactDeclaration} and \texttt{FactProperty} Concepts of the RSD to determine whether they were visible.

Our following approach created subclasses of the \texttt{FactDeclaration}, \texttt{FactProperty} and \texttt{File} Concepts.
This approach, however, requires running a macro on the code file to migrate \texttt{FactDeclaration}, \texttt{FactProperty} and \texttt{Files} nodes to \texttt{FilteredFact}, \linebreak\texttt{FilteredFactProperty}, and \texttt{FilteredFile} nodes.
...
This simple table has only the \texttt{name} property and the ``when'' children of the \texttt{Rule} nodes in the \texttt{File}.
...
At the top, we can see an immediate problem with a crosstab, and that is if we have the whole \texttt{File} included, the table will be very sparse.
...
The \texttt{RuleCollection} is a child of the \texttt{File} and holds a collection of \texttt{Rule} nodes.
...
Our solution was to develop a vertically orientated header cell and to use indentation to indicate if the cell is referring to just the \texttt{FactDeclaration} or a \texttt{FactDeclaration} and \texttt{FactProperty} combination.
...
We achieved this by referencing a \texttt{RuleVariable} name in the cell representing the \texttt{FactDeclaration} or \texttt{FactProperty} to which it is bound.
...
Thus, we decided to represent \texttt{FactDeclaration} selection with an image.
...
We use intentions to add the selection of a \texttt{FactDeclaration} or \texttt{FactProperty} to a \texttt{Rule}.
We also use intentions for binding \linebreak\texttt{RuleVariables}.

The major drawback of this design is that editing a \texttt{Rule} with a yet non-existent selection criteria became very clunky.
...
The RHS of the \texttt{Rule} appears in the action's column.
...
\begin{figure}[h]
    \centering
    \fbox{\includegraphics[width=0.66\textwidth]{Sections/images/spreadsheetIntentions.png}}
    \caption{Intention}
...
Because of the combinatorial explosive nature of truth tables, with 2\textsuperscript{n} possible combinations, we would limit the display to a max of 6 \linebreak\texttt{FactSelectors} and only show the paths that lead to the RHS execution.
...
The user experience is that once the \texttt{Rule} is selected, the developer, by pressing the up and down arrow keys, can step through the different \texttt{FactSelectors} (highlighted in yellow) and shown in the circuit diagram, thus showing how the \texttt{FactDeclaration} nodes relate to each other.
